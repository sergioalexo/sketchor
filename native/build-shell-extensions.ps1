<#
.SYNOPSIS
  Builds the Sketchor Explorer DXF thumbnail-handler DLL and stages it where
  the Tauri bundler picks it up as an installer resource.
...
.DESCRIPTION
  ...
  staged DLL when it validates bundle.resources), so a packaged Sketchor
  installer *ships* the thumbnail
  handler and registers it itself (see apps/web/src-tauri/installer-hooks.nsh).
  End users no longer run native/*/install*.ps1 by hand.

  Can also be run standalone to refresh the staged copy. Paths are resolved
  from $PSScriptRoot, so the working directory does not matter.

.PARAMETER SkipBuild
  Stage the already-built release DLL without rebuilding (needs a prior build).
#>
