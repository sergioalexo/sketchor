...
//! produces a geometry thumbnail for a real `.dxf` file — driving the
...
//!
//!   cargo run --release --example verify_shell_thumb -- file.dxf [out.png] [size]
...
fn main() {
    ...
    let path = args
        .get(1)
        .cloned()
        .expect("usage: verify_shell_thumb <file.dxf> [out.png] [size]");
